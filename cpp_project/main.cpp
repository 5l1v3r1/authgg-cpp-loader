...
int main(int, char**)
{
    ...
    {
        AuthGG::Initialize::AID = L""; //your AID
        AuthGG::Initialize::ProgramSecret = L"";// your program secret
        AuthGG::Initialize::Version = L"1.0"; //version of your app
        AuthGG::Initialize::Secret = L"very random sir"; //Any random string you want, preferred if its a bunch of random characters all in one, this is an extra security feature to prevent dll patching ontop of hashcheck.
        AuthGG::Initialize::Init(); // Initialization
    }
    else if (utils::is_dll_valid == false) 
    {
        exit(0);
    }
    bool freemode = AuthGG::Auth::Freemode();
    if (freemode)
    {
        if (AuthGG::Auth::Response != AuthGG::Initialize::Secret)
        {
            MessageBox(NULL, "An Error Ocurred", "ERROR", MB_OK);
            exit(0);
        }
        MessageBox(NULL, "Application is in freemode, redirecting", "OK", MB_OK);
        //do freemode stuff here
    }
    ...
    while (!glfwWindowShouldClose(window))
    {
...

        {
            ...
            if (ImGui::Button("login")) {
                ...
                if (AuthGG::Auth::Login(csharp_username, csharp_password, true)) {
                    if (AuthGG::Auth::Response != AuthGG::Initialize::Secret) {
                        MessageBox(NULL, "An Error Ocurred", "ERROR", MB_OK);
                        exit(0);
                    }
        ...
        if (register_window) {
            ...
            if (ImGui::Button("register")) {
                ...
                if (AuthGG::Auth::Register(csharp_username, csharp_password, csharp_email, csharp_token, true)) {
        ...
        if (activate_window) {
            ...
            if (ImGui::Button("activate")) {
                ...
                if (AuthGG::Auth::RedeemLicense(csharp_username, csharp_password, csharp_token, true)) {
